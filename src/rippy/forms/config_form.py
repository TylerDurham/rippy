...
from rippy.core.config import RippyConfig, ensure_path, read_config, write_config
...
class ConfigForm(App):
    ...
    BINDINGS = [(":w", "save", "Save the config."), (":q", "quit", "Quit the app.")]
    TITLE = "Rippy"
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Vertical(
            ...
            Input(
                id="movie_min_title_length",
                value=f"{cfg.makemkv.movie_min_title_length}",
            ),
            Label("Minium TV Show Title Length", classes="label"),
            Input(
                id="tv_show_min_title_length",
                value=f"{cfg.makemkv.tv_show_min_title_length}",
            ),
        )
        yield Horizontal(Label("NORMAL", id="lbl_command_status"), Footer())
