...
class MovieSettings(BaseModel):
    min_title_length: PositiveInt = Field(
        default=defaults.MOVIE_MIN_TITLE_LENGTH,
        description="The minimum title length for the movie.",
    )


class TVSettings(BaseModel):
    min_title_length: PositiveInt = Field(
        default=defaults.TV_SHOWS_MIN_EPISODE_LENGTH,
        description="The minimum title length for the tv show.",
    )
...
class RippySettings(BaseModel):
    ...
    makemkv: Annotated[
        MakeMKVSettings, Field(description="The makemkv settings section.")
    ]
...
def get_description(model: Type[BaseModel], field_name: str) -> str:
    ...
    return (
        f.description or f"{model.__name__}.{field_name} annotation has no description."
    )
...
def _from_dict(cls, data: dict):
    ...
    return cls(
        **{
            key: (
                _from_dict(fieldtypes[key], value)
                if dc.is_dataclass(fieldtypes[key])
                else value
            )
...
def write_config(
    model: RippySettings, config_file_path: str = defaults.CONFIG_FILE_PATH
):
    """Writes the configration settings to hard disk.

    By default, writes in the CONFIG_FILE_PATH.

    Args:

     - model: (RippySettings): The settings to save.
